...
partial struct MeleeAttackSystem : ISystem
{
    ...
    public void OnUpdate(ref SystemState state)
    {
        ...
        foreach ((RefRW<LocalTransform> localTransform,RefRW<MeleeAttack> meleeAttack,RefRW<Target> target,RefRW<UnitMover> unitMover) in 
            SystemAPI.Query<RefRW<LocalTransform>,RefRW<MeleeAttack>,RefRW<Target>,RefRW<UnitMover>>())
        {
            ...
            if(!isCloseEnoughToAttack)
            {
                ...
                float distanceExtraToRaycast = 4.0f;

                RaycastInput raycastInput = new RaycastInput
                {
                    Start = localTransform.ValueRO.Position,
                    End = localTransform.ValueRO.Position + directionToTarget * (meleeAttack.ValueRO.colliderSize * distanceExtraToRaycast),
            ...
            else
            {
                ...
                meleeAttack.ValueRW.timerMin -= SystemAPI.Time.DeltaTime;

                if (meleeAttack.ValueRO.timerMin > 0.0f)
                {
                    continue;
                }

                meleeAttack.ValueRW.timerMin = meleeAttack.ValueRO.timerMax;
