...
{
    public const float UNIT_DISTANCE_REACHED_SQ = 0.2f;
...
public partial struct UnitMoverJob : IJobEntity
{
    ...
    public void Execute(ref LocalTransform localTransform,ref UnitMover unitMover,ref PhysicsVelocity physicsVelocity)
    {
        ...
        if(math.distancesq(unitMover.targetPosition,localTransform.Position) > UnitMoverSystem.UNIT_DISTANCE_REACHED_SQ)
        {
            localTransform.Rotation = math.slerp(localTransform.Rotation, quaternion.LookRotation(movementDirection, math.up()), unitMover.rotationSpeed * deltaTime);
